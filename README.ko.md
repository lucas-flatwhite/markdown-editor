...
- 라이트/다크 테마 전환
- 영문/한국어 2개 언어 UI 지원
- 선택 언어를 `localStorage`에 저장하여 재방문 시 복원
- 선택 언어에 따라 `<html lang>` 속성 자동 동기화
- 샘플 마크다운 문서도 언어별(`en`/`ko`)로 제공
- 편집기-미리보기 싱크 스크롤
...
- 다국어: React Context 기반 커스텀 로컬라이제이션(`en`/`ko`)
...
- `DEBUG_COLLECTOR=true`로 개발 서버 실행 시 디버그 수집기 활성화
- 디버그 로그 출력 경로: `.debug-logs/`
